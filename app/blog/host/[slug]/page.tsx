...
import { getAllHostSlugs, getHostBySlug } from '@/lib/hosts'
...
export default function HostPage({ params }: { params: { slug: string } }) {
  ...
  const relatedPosts = host.related
    ...
    <div className="min-h-screen relative z-10">
      ...
      <main className="relative max-w-5xl mx-auto px-4 py-10">
        <nav className="text-sm text-muted-foreground mb-8">
          ...
          <Link href="/blog/hosts" className="hover:text-foreground transition-colors">Hosting</Link>
          <span className="mx-2 opacity-30">/</span>
          <span className="text-foreground">{host.title}</span>
        </nav>

        <article className="glass rounded-2xl p-6 sm:p-8">
          <header className="mb-6">
            <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight mb-3">{host.title}</h1>
            <p className="text-muted-foreground">{host.description}</p>
          </header>

          <div className="grid gap-6 sm:grid-cols-2">
            <div>
              <h2 className="font-semibold mb-3">Typical limits</h2>
              <ul className="space-y-2 text-sm text-muted-foreground">
                {host.limits.map((item) => (
                  <li key={item}>• {item}</li>
                ))}
              </ul>
            </div>
