...
import { getPostsByTopic, topicConfigs, TopicKey } from '@/lib/posts'

export function generateStaticParams() {
  return Object.keys(topicConfigs).map((topic) => ({ topic }))
}

export function generateMetadata({ params }: { params: { topic: string } }): Metadata {
  const key = params.topic as TopicKey
  const config = topicConfigs[key]
  if (!config) {
...
export default function TopicPage({ params }: { params: { topic: string } }) {
  ...
  const posts = getPostsByTopic(key)
    ...
    <div className="min-h-screen relative z-10">
      ...
      <main className="relative max-w-5xl mx-auto px-4 py-12">
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-semibold tracking-tight mb-2">{config.title}</h1>
