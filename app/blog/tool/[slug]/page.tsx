...
export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  ...
  return {
    title: `${tool.title} — SQLSplit`,
    description: tool.description,
    alternates: { canonical: `/blog/tool/${params.slug}` },
  }
}
...
export default function ToolPage({ params }: { params: { slug: string } }) {
  ...
  const relatedPosts = tool.related
    ...
    <div className="min-h-screen relative z-10">
      ...
      <main className="relative max-w-5xl mx-auto px-4 py-10">
        <nav className="text-sm text-muted-foreground mb-8">
          ...
          <Link href="/blog/tools" className="hover:text-foreground transition-colors">Tools</Link>
          <span className="mx-2 opacity-30">/</span>
        ...
        <article className="glass rounded-2xl p-6 sm:p-8">
          <header className="mb-6">
            <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight mb-3">{tool.title}</h1>
            <p className="text-muted-foreground">{tool.description}</p>
          </header>

          <div className="grid gap-6 sm:grid-cols-2">
            <div>
              <h2 className="font-semibold mb-3">Best for</h2>
